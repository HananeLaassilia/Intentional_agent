...
class IntelligentAgent:
    ...
    def a_star_pathfind(self, start, goal, obstacles):
        ...
        while heap:
            ...
            for dx, dy in [(-1,0),(1,0),(0,-1),(0,1)]:
                ...
                neighbor = (nx, ny)
                if 0 <= nx < GRID_WIDTH and 0 <= ny < GRID_HEIGHT and neighbor not in obstacles and neighbor not in visited:
                    new_g = g_score + 1
    ...
    def select_next_task(self):
        for i, task in enumerate(self.task_queue):
            if self.a_star_pathfind(self.pos, task.box_pos, set()):
                return self.task_queue.pop(i)
        return None

    def update_intelligence(self, boxes, holes, obstacles):
        ...
        if not self.carrying and not self.current_task:
            ...
            if self.current_task:
                ...
                self.path = self.a_star_pathfind(self.pos, self.current_task.box_pos, obstacles)
            else:
            ...
            if targets:
                ...
                self.path = self.a_star_pathfind(self.pos, target[0], obstacles)
